...
```
    ...
    ## Installing package into '/home/willibro/R/x86_64-pc-linux-gnu-library/4.1'
...
``` {r-learning-the-quantile-function}
quantile(gapminder$lifeExp)
quantile(gapminder$lifeExp, probs = 0.5)
quantile(gapminder$lifeExp, probs = c(0.25, 0.75))
```
...
``` {r-what-does-probs-do}
quantile(gapminder$lifeExp, probs = c(0.78))
```
...
quant_calculator<- function(x){
  tf<-quantile(x, probs = c(0.25))
  sf<-quantile(x, probs = c(0.75))
  totaltf <- (sf+tf)
}
```

``` r
gggg <- quant_calculator(1:12)
```
